...
- **F5 / Ctrl+Enter 执行当前语句**：有选区时优先执行选区，否则只执行光标所在语句；
  「运行全部」才会执行整个编辑器，多语句脚本逐条显示结果，SQL Server `GO` 批次和过程体边界会正确识别
- **显式事务控制**：查询标签可关闭「自动提交」，使用「开始 / 提交 / 回滚」控制该标签独占的物理数据库会话；
  活动事务期间禁止切换连接/数据库/模式，关闭标签或连接会先回滚。关闭仍有活动事务或查询的连接时会明确二次确认。
  PostgreSQL 查询会在同一物理会话应用所选模式（`pg_catalog` 优先），SQL Server 补全使用模式限定名；
  恢复草稿指向的数据库已不存在时会禁用执行，绝不会静默改到第一个数据库。ClickHouse 当前连接模式保持仅自动提交
...
- 索引编辑：增删索引、唯一索引
- 外键编辑：名称、本表栏位、引用 Schema/表/栏位、`ON UPDATE` / `ON DELETE` 动作；
  支持 MySQL/MariaDB、PostgreSQL、SQL Server、OceanBase，SQLite 新建表可配置、已有表明确提示需要重建
- 约束编辑：可视化管理 `UNIQUE` 和 `CHECK`，不支持直接修改约束的数据库会在 SQL 预览中明确降级
...
### 其它
- **工作区自动恢复**：通过本地主进程原子文件保存上次打开的连接、标签顺序、活动标签、查询 SQL 草稿、查询连接/数据库、
  表格筛选/排序/分页/视图状态、表设计草稿和侧栏/查询分隔条尺寸；重启后自动恢复，不保存查询结果集。
  临时密码连接恢复时仍会要求重新输入；连接暂时不可用时保留对应标签，连接成功后继续恢复；从文件打开的 SQL
  恢复后会变成独立草稿，重新保存时再次通过系统文件对话框授权。缺少连接定义的新表/脏设计草稿会继续作为孤立草稿保留，
  长对象标识不截断；恢复写入采用顺序队列，退出前会确认最后一版草稿已实际落盘
- 多标签页工作区（Ctrl+W 关闭），未保存更改或活动事务关闭时提示
- 连接树、对象页和工作标签共用统一的“当前连接 / 数据库 / 模式 / 对象”上下文；多连接场景下
  全局命令不会再静默选择第一个连接
