...
async function main() {
  ...
  const gridSource = fs.readFileSync(path.join(__dirname, '..', 'src', 'renderer', 'js', 'grid.js'), 'utf8');
  ...
  assert.doesNotMatch(querySource, /autoCommitInput/);
  assert.match(querySource, /transactionId,\s*\n\s*};/);
  assert.match(querySource, /finishTransactionBefore\('切换连接'\)/);
  assert.match(querySource, /finishTransactionBefore\('关闭查询'\)/);
  assert.match(querySource, /finishResultEditsBefore\('关闭查询'\)/);
  assert.match(querySource, /saveResultEditor\(editor\)/);
  assert.match(querySource, /editTarget \? editTarget\.pk : \[\]/);
  assert.match(querySource, /保存修改 \(Ctrl\+S\)/);
  assert.match(gridSource, /const headerTip = c\.comment/);
  assert.match(gridSource, /title: headerTip/);
  assert.match(gridSource, /!e\.ctrlKey && !e\.metaKey && !e\.shiftKey && \(this\.editable \|\| isNew\)/);
  assert.match(gridSource, /this\._beginEdit\(td, r, i, isNew\)/);
  assert.match(gridSource, /this\.opts\.onSave/);
  assert.match(gridSource, /acceptChanges\(\)/);
  assert.match(tabsSource, /export async function runBeforeCloseGuards/);
  assert.match(querySource, /dbUnavailable/);
  assert.match(querySource, /const schemaSel = el\('select'/);
  assert.match(querySource, /window\.api\.db\.schemas\(connId,\s*db\)/);
  assert.match(querySource, /schemaSel\.addEventListener\('change'/);
  assert.match(querySource, /connId:\s*queryConnId,\s*db,\s*schema,\s*sql/);
