...
function columnDef(adapter, dialect, col, opts = {}) {
  ...
  if (col.autoInc) {
    ...
    if (dialect === 'postgres') {
      // 标识列（GENERATED … AS IDENTITY）是 PostgreSQL 10 才有的语法，
      // 9.x 只能用 serial / bigserial（它本质是「整型 + 默认值取自动建的序列」）。
      const legacy = adapter.serverVersionNum && adapter.serverVersionNum < 100000;
      if (legacy) {
        const wide = /big|int8/i.test(col.type || '');
        parts[1] = wide ? 'bigserial' : 'serial';   // serial 自带类型，替换掉原类型
      } else {
